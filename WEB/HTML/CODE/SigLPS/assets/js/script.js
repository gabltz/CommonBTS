...
function generateSignature() {
    ...
    let imageHTML = ''; // Initialiser la variable pour l'image HTML
    const includeImage = document.getElementById("includeImage").checked; // Vérifier si l'utilisateur a coché la case
    if (includeImage) { // Si la case est cochée, inclure l'image dans la signature
        imageHTML = `
  ...
  // Pour éviter de garder un carré blanc lorsque l'on décoche "inclure l'image", placer tout le <td> de l'image dans la variable imageHTML dans la fonction generateSignature()
...
function openSignature() {
  const signatureTextarea = document.getElementById("signatureResult").innerHTML; // Récupère le contenu de la signature
  const signatureWindow = window.open("", "_blank"); // Ouvre une nouvelle fenêtre
  ...
  signatureWindow.document.write(signatureTextarea); // Écrit le contenu de la signature dans la nouvelle fenêtre
  signatureWindow.document.write('</body>');
...
document.addEventListener('DOMContentLoaded', (event) => {
  // Vérifie si le bouton existe avant d'attacher le gestionnaire d'événements
  const toggleButton = document.getElementById('toggleExpertOptions');
  if (toggleButton) {
      toggleButton.addEventListener('click', toggleExpertOptions);
  }
});

// Fonction pour afficher/cacher la section "Expert"
function toggleExpertOptions() {
  const expertOptions = document.getElementById('expertOptions');
  if (expertOptions.style.display === 'none' || expertOptions.style.display === '') {
      expertOptions.style.display = 'block';
      document.getElementById('toggleExpertOptions').textContent = 'Cacher les options Expert';
  } else {
      expertOptions.style.display = 'none';
      document.getElementById('toggleExpertOptions').textContent = 'Afficher les options Expert';
  }
}

function copySignatureHTML() {
  const signatureHTML = document.getElementById("signatureResult").innerHTML;
...
function copyRenderedSignature() {
  ...
  try {
      ...
      const msg = successful ? 'réussie' : 'échouée';
      alert('Copie de la signature rendue ' + msg);
  } catch (err) {
      alert('Échec de la copie');
  }

  window.getSelection().removeAllRanges(); // Désélectionne la signature après la copie
}
